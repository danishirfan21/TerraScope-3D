...
const InvestorDashboard = () => {
    ...
    return (
        <Box className="investor-dashboard fade-in" sx={{ position: 'absolute', top: 80, right: 20, width: 320, zIndex: 1000 }}>
            {/* KPI Section */}
            <Paper className="glass-effect" sx={{ p: 2, mb: 2 }}>
                <Typography variant="overline" sx={{ color: '#2196f3', fontWeight: 'bold' }}>Enterprise Intelligence</Typography>
            ...
            {/* Momentum Gauge */}
            <Paper className="glass-effect" sx={{ p: 2, mb: 2, textAlign: 'center' }}>
                ...
                <Box sx={{ position: 'relative', display: 'inline-flex' }}>
                    ...
                    <Box sx={{ top: 0, left: 0, bottom: 0, right: 0, position: 'absolute', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Typography variant="h6" component="div" color="white">{marketMomentum}</Typography>
                    </Box>
                </Box>
                <Typography variant="body2" sx={{ mt: 1, color: marketMomentum > 70 ? '#4caf50' : 'gray' }}>
                    {marketMomentum > 70 ? 'High Growth Phase' : 'Steady Market'}
                </Typography>
            </Paper>

            {/* Top 5 Opportunities */}
            <Paper className="glass-effect" sx={{ p: 2 }}>
                <Typography variant="subtitle2" sx={{ mb: 1, display: 'flex', alignItems: 'center', color: '#ffeb3b' }}>
                    <Stars sx={{ mr: 1, fontSize: 18 }} /> TOP OPPORTUNITIES
                </Typography>
                ...
                <List dense>
                    {topOpportunities.map((op, idx) => (
                        ...
                        >
                            <ListItemText
                                primary={op.address || op.properties?.address}
                                secondary={`ROI: ${((op.yield || op.properties?.yield || 0) * 100).toFixed(1)}% | Score: ${Math.round(op.investmentScore)}`}
                                primaryTypographyProps={{ color: 'white', variant: 'body2' }}
                                secondaryTypographyProps={{ color: 'gray', variant: 'caption' }}
                            />
                        </ListItem>
                    ))}
                </List>
